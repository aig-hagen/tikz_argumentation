...
\usepackage[nohyperref]{doc}                    % Showing Commands (must load before tcolorbox); nohyperref avoids doc auto-loading hypdoc, which resets hyperref's colorlinks/hidelinks at begin-document
...
\begin{figure}[ht]
    ...
    \begin{af}[namestyle=math]
        ...
        \annotation[right=of a5]{$a\lor b$}
        ...
        \attack[above]{a3}{a2}($3$)
        \attack[below,incomplete]{a4}{a5}($x$)
        \attack{a5}{a10}
        \setattack{a9,a10}{a4}
...
\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Documentation for Version 1.8 [2026/07/08]}\label{sec:documentation}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
It builds on the \tikzname package for drawing the argumentation graphs, providing a simplified, high-level syntax for common argumentation constructs while retaining full \tikzname compatibility and customisation options.

The package supports a wide range of features including:
\begin{itemize}
    \item argument nodes with various visual styles and automatic or manual indexing,
    \item attack and support edges, including collective (hyperedge) attacks and supports,
    \item annotated edges and argument annotations,
    \item labelling and referencing of argumentation frameworks,
    \item size presets for two-column layouts, and
    \item beamer overlay specifications for creating presentations.
\end{itemize}
...
To create a directed attack between two argument nodes, you can use the following command.
The parameters \mopt{arg1} and \mopt{arg2} are the identifiers of the two arguments.\\

\noindent\cs{attack [\oopt{options}] (\popt{id}) \{\mopt{argId1}\} \{\mopt{argId2}\} (\popt{annotation})}
\begin{description}
    \item[\oopt{options}] (optional) a list of \tikzname style parameters.
    \item[\popt{id}] (optional) the identifier of the attack edge. Per default, when omitted, attacks are automatically assigned an identifier of the form: \textsf{att1}, \textsf{att2}, \dots. This identifier always names a coordinate on the edge -- whether or not an \popt{annotation} is given -- which can be used like any other coordinate, e.\,g.\ for relative positioning or as the source/target of another attack.
    \item[\mopt{arg1}] Identifier of the attacking argument.
    \item[\mopt{arg2}] Identifier of the attacked argument.
    \item[\popt{annotation}] (optional) annotation text placed on the attack edge. Its placement can be controlled via \oopt{options}, e.\,g., \textsf{above}, \textsf{below}, \textsf{left} or \textsf{right}.\\
\end{description}
...
\noindent\cs{dualattack [\oopt{options}] (\popt{id1}) (\popt{id2}) \{\mopt{argId1}\} \{\mopt{argId2}\}}
\begin{description}
    \item[\oopt{options}] (optional) a list of \tikzname style parameters.
    \item[\popt{id1},\popt{id2}] (optional) identifiers for the two created attack edges (\mopt{argId1}$\to$\mopt{argId2} and \mopt{argId2}$\to$\mopt{argId1}, respectively). Defaults follow the same automatic numbering as \verb|\attack|.
    \item[\mopt{arg1}] Identifier of the first argument.
    \item[\mopt{arg2}] Identifier of the second argument.
\end{description}

\noindent\cs{selfattack [\oopt{options}] (\popt{id}) \{\mopt{argId}\}}
\begin{description}
    \item[\oopt{options}] (optional) a list of \tikzname style parameters.
    \item[\popt{id}] (optional) identifier for the created attack edge.
...
\noindent\cs{support [\oopt{options}] (\popt{id}) \{\mopt{argId1}\} \{\mopt{argId2}\} (\popt{annotation})}
\begin{description}
    \item[\oopt{options}] (optional) a list of \tikzname style parameters.
    \item[\popt{id}] (optional) the identifier of the support edge, named and defaulted the same way as for \verb|\attack| (\textsf{supp1}, \textsf{supp2}, \dots).
    \item[\mopt{arg1}] Identifier of the supporting argument.
    \item[\mopt{arg2}] Identifier of the supported argument.
    \item[\popt{annotation}] (optional) annotation text placed on the support edge.\\
\end{description}

To draw a \emph{collective} (hyperedge) attack or support from a \emph{set} of arguments to a single target, the following two commands are provided.
Each command places a \emph{junction point} on the line between the barycenter of the source arguments and the target node, at a clearance of \texttt{junction dist} beyond the target's border.
A Bézier curve is drawn from each source to this junction, and a single arrowhead edge is drawn from the junction to the target.\\

\noindent\cs{setattack [\oopt{options}] (\popt{id}) \{\mopt{sourceList}\} \{\mopt{argId}\} (\popt{annotation})}
\begin{description}
    \item[\oopt{options}] (optional) a list of \tikzname style parameters.
    \item[\popt{id}] (optional) the identifier of the arrow from the junction to the target, named and defaulted the same way as for \verb|\attack|. Additionally, each individual curve from a source argument to the junction gets its own identifier \popt{id}\textsf{-1}, \popt{id}\textsf{-2}, \dots, in the order the sources are listed in \mopt{sourceList}.
    \item[\mopt{sourceList}] Comma-separated list of identifiers of the attacking arguments.
    \item[\mopt{argId}] Identifier of the attacked argument.
    \item[\popt{annotation}] (optional) annotation text placed on the arrow from the junction to the target.\\
\end{description}

\noindent\cs{setsupport [\oopt{options}] (\popt{id}) \{\mopt{sourceList}\} \{\mopt{argId}\} (\popt{annotation})}
\begin{description}
    \item[\oopt{options}] (optional) a list of \tikzname style parameters.
    \item[\popt{id}] (optional) the identifier of the arrow from the junction to the target, named and defaulted the same way as for \verb|\support|. Additionally, each individual curve from a source argument to the junction gets its own identifier \popt{id}\textsf{-1}, \popt{id}\textsf{-2}, \dots, in the order the sources are listed in \mopt{sourceList}.
    \item[\mopt{sourceList}] Comma-separated list of identifiers of the supporting arguments.
    \item[\mopt{argId}] Identifier of the supported argument.
    \item[\popt{annotation}] (optional) annotation text placed on the arrow from the junction to the target.\\
\end{description}

The position of the junction point relative to the target node border can be controlled via the \texttt{junction dist} option (see Section~\ref{sec:options}), either globally, per \texttt{af} environment, or per command, e.\,g., \verb|\setattack[junction dist=2ex]{...}{...}|.
...
\begin{example}~
    ...
    \begin{minipage}{0.3\textwidth}
        ...
        \begin{af}[namestyle=math]
            ...
            \argument[right=of a1]{c}
            \argument[below=of a3]{d}
...
The following command can be used to create a text annotation (e.g.\ a value or a condition) inside the argumentation framework.
It behaves essentially exactly like the \cs[0cm]{argument} command: there is no default position, so place it either relative to another node, e.g.\ \verb|[right=of a5]|, or at absolute coordinates via \texttt{at (\popt{posX}, \popt{posY})}.\\

\noindent\cs{annotation [\oopt{options}] (\popt{id}) \{\mopt{value}\} at (\popt{posX}, \popt{posY})}
\begin{description}
    \item[\oopt{options}] (optional) a list of \tikzname style parameters and/or relative positioning parameters.
    \item[\popt{id}] (optional) Identifier of the annotation node. If omitted, an identifier of the form \textsf{annotation1}, \textsf{annotation2}, \dots is generated automatically.
...
\noindent\cs{setattackstyle \{\mopt{style parameters}\}}

\noindent\cs{setsupportstyle \{\mopt{style parameters}\}}

\noindent\cs{setannotationstyle \{\mopt{style parameters}\}}\\
...
\begin{example}~
    ...
    \begin{minipage}{0.22\textwidth}
        ...
        \setannotationstyle{node distance=1ex}
        \begin{af}
            \label{af:8}
            \argument{a}
            \argument[right=of a1]{b}
...
\begin{description}
    ...
    \item[supportstyle] (default \texttt{double}) Globally sets the appearance of the support edges.
    The package comes with three available options: \texttt{marked}, \texttt{dashed} and \texttt{double}.
    Detailed descriptions of these options can be found below.

    \item[junction dist] (default \texttt{1ex}) Sets the minimum clearance between the junction point and the border of the target node for the hyperedge commands \verb|\setattack| and \verb|\setsupport|.
    Can also be set locally per \texttt{af} environment, e.\,g., \verb|\begin{af}[junction dist=2ex]|.
...
\begin{align*}
    \mathsf{marked} &\quad \text{Same tip as attack arrow, perpendicular mark on arrow line.}\\
...
\begin{figure}[!h]
    \begin{subfigure}{0.32\textwidth}
        \centering
        \begin{af}[supportstyle=marked]
            \argument{a}
            \argument[right=of a1]{b}

            \support[]{a1}{a2}
        \end{af}
        \caption{\textsf{supportstyle}=\textit{marked}}
        \label{fig:supportstyle_marked}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.3\textwidth}
        \centering
    ...
    \end{subfigure}
    \caption{Available options for \textsf{supportstyle}. Note that for \textit{marked} and \textit{dashed} the arrow tip of the selected \textsf{attackstyle} will be used.}
...
\begin{table}[ht]
        ...
        \begin{tabular}{l|l}
            ...
            \textsf{support marked} & attack,postaction=\{decorate,decoration=\{\dots\}\}\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{[v1.8 2026/07/08]}
\begin{itemize}
    \item Added an optional \texttt{(id)} parameter to \verb|\attack|, \verb|\support|, \verb|\selfattack|, \verb|\dualattack|, \verb|\setattack| and \verb|\setsupport|, naming a coordinate on the edge that can be referenced like any other node. Defaults to automatically numbered identifiers (\textsf{att1}, \textsf{att2}, \dots for attacks; \textsf{supp1}, \textsf{supp2}, \dots for supports).
    \item Attack and support edges now always create a referenceable coordinate, even without an \popt{annotation}; previously no node was created in that case.
    \item \verb|\setattack|/\verb|\setsupport| additionally name each individual source-to-junction curve (\texttt{id-1}, \texttt{id-2}, \dots).
    \item Fixed a bug where automatically generated identifiers for attacks, supports and annotations were not reset between separate \texttt{af} environments (only argument identifiers were).
\end{itemize}

\subsection*{[v1.7 2026/06/20]}
\begin{itemize}
    \item Added command \verb|\setattack{ }{ }| for drawing collective attacks from a set of arguments to a single target.
    \item Added command \verb|\setsupport{ }{ }| for drawing collective support from a set of arguments to a single target.
    \item Renamed \textsf{supportstyle} option \texttt{standard} to \texttt{marked}; changed default from \texttt{standard} to \texttt{double}.
    \item Added package and command-level option \texttt{junction dist} to control distance of the hyperedge junction point to the target node border.
    \item Adjusted \textsf{annotation} style: font changed to \verb|\scriptsize|, default node distance reduced to \texttt{4ex}.
\end{itemize}
